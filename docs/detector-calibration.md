...
因此模型质量结果状态仍为 `blocked_model_quality`：不修改线上二分类阈值，不运行
封存 test。产品层按当前决策启用带显著警告的实验性五级风险展示；这不代表模型已
通过部署质量门槛。
...
这些边界对应接口中的实验性 `risk_level`。模型质量门槛仍然失败，校准产物继续保留
`deployable=false`；接口返回 `risk_level_is_experimental=true`，前端必须显示
“实验性”及 DDPM/Pix2Pix 区分度待改进的提示。二分类 `threshold` 继续保持 `0.5`。
