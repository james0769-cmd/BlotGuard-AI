...
DEFAULT_CODE_DIR = ROOT / "sam_lora_aigc_detect"
...
def default_image():
    patterns = [
        ...
        DEFAULT_CODE_DIR / "original_image.png",
    ]
...
def load_image(cv2, torch, sam, image_path, device):
    ...
    return sam.preprocess(tensor).unsqueeze(0)


def main():
    parser = argparse.ArgumentParser(description="Smoke test for the LoRA-SAM AIGC detector.")
    ...
    class DetectLoRASam(nn.Module):
        def __init__(self, sam_model, rank, lora_layers):
            ...
            self.sam = sam_model
            self.classifier = FCN_Classifier(num_classes=1)
    ...
    model = DetectLoRASam(sam, args.rank, parse_layers(args.lora_layers))
    model.load_lora_parameters(str(args.lora_weight))
    model.to(device).eval()

    image_tensor = load_image(cv2, torch, model.sam, image_path, device)
    with torch.no_grad():
        logit = model(image_tensor).flatten()[0]
